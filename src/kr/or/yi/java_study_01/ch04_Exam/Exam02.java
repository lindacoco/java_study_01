...
public class Exam02 {
	...
	public static void main(String[] args) {
		...
		System.out.println(Arrays.toString(stdArr)); //null로 초기화됨을 확인
	   ...
	   findStd.setStdNo(15);
	   idx = indexOf(stdArr, findStd); //존재하지 않는 학생이므로 -1을 예상한다
	...
	private static void initArr(Student[] arr) {
		...
		String[] names = {"권수진","정아름","장현서","황태원","현재승","배진우","이동주","황하나","유경진","이상원","박인선"};
		
		for(int i =0; i<arr.length; i++) {
			arr[i] = new Student(i+1,names[i],
					rnd.nextInt(60)+41,//(int)(Math.random()*60)+41,
					rnd.nextInt(60)+41,//(int)(Math.random()*60)+41,
					rnd.nextInt(60)+41); //(int)(Math.random()*60)+41);
